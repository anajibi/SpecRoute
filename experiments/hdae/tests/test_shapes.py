...
def test_full_shapes():
    ...
    h=HDAEConfig(EncoderHierarchyConfig(tap_resolutions=[8,16],level_dims=[10,6]),ConditioningConfig(style_ch=16,latent_drop_prob=0.0))
    m=HierarchicalAutoencModel(c,h);x=torch.randn(2,3,32,32);e=m.encode(x);out=m(x,torch.zeros(2,dtype=torch.long),x_start=x)
    assert e['cond'].shape==(2,16) and len(e['zs'])==2 and e['null_mask'].shape==(2,2) and out.pred.shape==x.shape
