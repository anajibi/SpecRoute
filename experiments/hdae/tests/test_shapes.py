...
def test_full_shapes():
    ...
    h=HDAEConfig(EncoderHierarchyConfig(hier_tap_block_ids=[0,"mid"],hier_level_dims=[10,6],level_dims=[10,6],hier_block_to_level=[0,0,1,1],n_decoder_output_blocks=4),ConditioningConfig(style_ch=16,latent_drop_prob=0.0))
    m=HierarchicalAutoencModel(c,h);x=torch.randn(2,3,32,32);e=m.encode(x);out=m(x,torch.zeros(2,dtype=torch.long),x_start=x)
    assert e['cond'].shape==(2,32) and len(e['zs'])==2 and e['null_mask'].shape==(2,2) and out.pred.shape==x.shape


def test_encode_with_nulls_forces_requested_level():
    c=BeatGANsAutoencConfig(image_size=32,in_channels=3,model_channels=8,out_channels=3,num_res_blocks=1,
      attention_resolutions=(),channel_mult=(1,2),embed_channels=32,resnet_two_cond=True,enc_out_channels=16,
      enc_num_res_block=1,enc_channel_mult=(1,2,4),enc_attn_resolutions=(),enc_pool='adaptivenonzero')
    h=HDAEConfig(EncoderHierarchyConfig(hier_tap_block_ids=[0,"mid"],hier_level_dims=[10,6],level_dims=[10,6],hier_block_to_level=[0,0,1,1],n_decoder_output_blocks=4),ConditioningConfig(style_ch=16,latent_drop_prob=0.0))
    m=HierarchicalAutoencModel(c,h);x=torch.randn(2,3,32,32);e=m.encode_with_nulls(x,[0])
    assert e['null_mask'].tolist() == [[True, False], [True, False]]
