...
def test_outputs_gradients_and_flat():
    m=HierarchicalSemanticEncoder(conf(),[0,"mid"],[5,3]); x=torch.randn(2,3,32,32,requires_grad=True);zs=m(x)
    ...
    assert HierarchicalSemanticEncoder(conf(),["mid"],[7])(x.detach())[0].shape==(2,7)


def test_invalid_tap():
    with pytest.raises(ValueError): HierarchicalSemanticEncoder(conf(),[99],[4])
