...
def forward_cumulative_rows(num_levels):
    return [(f"forward_{'_'.join(f'Z{i}' for i in range(n))}", list(range(n))) for n in range(1, num_levels + 1)]
...
def main():
    ...
    meta = {"output": str(output), "num_levels": num_levels, "T": T, "row_labels": labels,
