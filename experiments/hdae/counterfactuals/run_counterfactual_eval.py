...
def main():
    ...
    for batch in loader:
        ...
        for direction_sign in directions:
            ...
            for local_i, index in enumerate(batch["index"][:len(x)].tolist()):
                target_before_bin = before[local_i, target_index] >= 0.5
                target_after_bin = after[local_i, target_index] >= 0.5
                rec = {"index": int(index), "direction": direction_sign,
                       ...
                       "target_flip": bool(target_before_bin != target_after_bin)}
                for j, name in enumerate(attr_names):
                    before_bin = before[local_i, j] >= 0.5
                    after_bin = after[local_i, j] >= 0.5
                    rec[f"before_{name}"] = float(before[local_i, j])
                    rec[f"after_{name}"] = float(after[local_i, j])
                    rec[f"delta_{name}"] = float(delta[local_i, j])
                    rec[f"flip_{name}"] = bool(before_bin != after_bin)
                all_rows.append(rec)
            if first_grid is None:
                first_grid = (x.add(1).div(2).detach().cpu(), cf.clamp(0, 1).detach().cpu(), direction_sign)
        seen += len(x)
    ...
    for direction_sign in directions:
        ...
        before = np.asarray([[r[f"before_{name}"] for name in attr_names] for r in rows])
        after = np.asarray([[r[f"after_{name}"] for name in attr_names] for r in rows])
        summaries[direction_sign] = summarize_attribute_changes(before, after, target_index)
    ...
    if first_grid is not None:
        ...
        save_labeled_grid([orig_row, cf_row], ["original", f"cf_{args.attribute}_{direction_label}"],
