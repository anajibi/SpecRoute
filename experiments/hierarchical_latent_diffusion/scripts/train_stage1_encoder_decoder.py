...
def main():
 ...
 for epoch in range(epochs):
  ...
  for batch in image_loader(cfg,split='train',max_images=a.max_images):
   x=batch_images(batch,dev);zs=enc(dino(x));pred=dec(zs);true=vae.encode(x);loss,_=reconstruction_loss(pred,true,zs=zs,lambda_compress=cfg['stage1'].get('lambda_compress',1e-4));opt.zero_grad();loss.backward();torch.nn.utils.clip_grad_norm_(list(enc.parameters())+list(dec.parameters()),1.);opt.step();total+=loss.item()
