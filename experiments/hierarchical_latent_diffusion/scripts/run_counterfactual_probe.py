...
def main():
 ...
 with torch.no_grad():
  for batch in image_loader(cfg,split='test',shuffle=False,max_images=a.max_images):
   x=batch_images(batch,dev);ids=batch_image_ids(batch);zs=enc(dino(x));base=vae.decode(dec(zs));base_f=dino(base)
   for level in range(cfg['K']):
    mod=list(zs);cond=None if level==0 else torch.cat(mod[:level],-1);mod[level]=pri.levels[level].sample(x.shape[0],cond,dev,a.steps);img=vae.decode(dec(mod));sim=cosine_similarity(base_f,dino(img));mse=mse_distance(base,img)
    for i,name in enumerate(ids):append_csv(out/'counterfactual.csv',{'image_id':name,'K':cfg['K'],'level':level+1,'dinov2_sim':sim[i].item(),'image_mse':mse[i].item()})
